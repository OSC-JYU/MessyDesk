...
module.exports = class Graph {
	...
	async getUserFileMetadata_old(file_rid, me_email) {
		...
		.has(\"id\", \"${me_email}\")
		.select(\"f\")`
		var file_response = await web.gremlin(query)
	...
	async getGraph_old(body) {
		...
		return web.cypher(body.query, 'graph', schema_relations, body.current)
	}

	async getSchemaRelations() {
		var schema_relations = {}
		var schemas = await web.cypher('MATCH (s:Schema)-[r]->(s2:Schema) return type(r) as type, r.label as label, r.label_rev as label_rev, s.label as from, s2.label as to, r.tags as tags, r.compound as compound')
	...
	async getDataWithSchema(itemid) {
		...
		var data = await web.cypher(`MATCH (source) WHERE id(source) = "#${itemid}" OPTIONAL MATCH (source)-[rel]-(target)  return *`)
